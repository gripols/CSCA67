...
\label{cha:Predicates and Quantifiers}


\section{Variables and Sets}

\textbf{Predicates} are statements that contain variables and become
either true or false when specific values are substituted.
\textbf{Quantifiers} allow us to express properties of collections of
objects, such as ``for all elements'' or ``there exists an element.''
...
\label{sub:Quantifier Negation}

\begin{exercise}[Translation and Negation Exercise]
    \begin{itemize}
        \item Every CS Major takes Discrete Maths
        \begin{itemize}
            \item $\neg \forall x, C(x) \to M(x)$
            \item $\equiv \exists x \neg(C(x) \to M(x))$
            \item $\equiv \exists x \neg(\neg C(x) \lor M(x))$
            \item $\equiv \exists x, \neg \neg C(x) \land \neg M(x)$
            \item $\exists x, C(x) \land \neg M(x)$
            \item Some (there is at least one) CS major who does not take discrete math.
        \end{itemize}
        \item Some pets do not like walks
            \begin{itemize}
                \item $\neg (\exists x, P(x) \land \neg W(x))$
                \item $\equiv \forall x, \neg (P(x) \land \neg W(x))$
                \item $\equiv \forall x, \neg P(x) \lor \neg \neg W(x)$
                \item $\forall x, \neg P(x) \lor W(x)$
                \item $\equiv \forall x, P(x) \to W(x)$ 
                \item All pets like walks. 
            \end{itemize}
     \end{itemize}
\end{exercise}

\begin{exercise}[Negate the Expression and Simplify the Result]
   \[
   \forall x \exists y \forall z, P(x, y) \to (Q(y, z)) \land \exists w, R(z, w)
   .\] 

   Ow, this looks painful. How do we solve? Before we do anything, understand
   the structure of your problem. Then, we negate and move accordingly:
   \[
   \equiv \exists x \forall y \exists z, \neg(P(x,y)\to (Q(y,z)\land \exists w,
   R (z, w)))
   .\] (Quantifier Negation)
   \[
   ...
   .\] (Conditional) 
   \[
   \equiv \exists x \forall y \exists z, P(x,y) \land \neg (Q(y, z) \land
   \exists w, R(z,w))
   .\] (De Morgan's)
   \[
   \equiv \exists x \forall y \exists z, P(x,y) \land (\neg Q(y, z) \lor \forall
   w, \neg R(z,w))
   .\] (De Morgan's + Quantifier Negation) 
\end{exercise}
